...
class SAPTableActionButtons extends StatelessWidget {
  const SAPTableActionButtons({
    super.key,
    this.view = false,
    this.edit = true,
    this.delete = true,
      ...
      children: [
        if (view)
          IconButton(
            ...
            icon: const Icon(Iconsax.eye, color: SAPColors.darkerGrey),
          ),
        if (edit)
          IconButton(
            onPressed: onEditPressed,
            icon: const Icon(Iconsax.pen_add, color: SAPColors.primary),
          ),
        if (delete)
          IconButton(
            onPressed: onDeletePressed,
            icon: const Icon(Iconsax.trash, color: SAPColors.error),
          ),
      ],
    );
  }
