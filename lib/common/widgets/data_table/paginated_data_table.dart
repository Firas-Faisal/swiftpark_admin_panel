...
class SAPPaginatedDataTable extends StatelessWidget {
  const SAPPaginatedDataTable({
    super.key,
    ...
    this.dataRowHeight = SAPSizes.xl * 2,
      ...
      child: Theme(
        ...
        child: PaginatedDataTable2(
          ...
          headingRowColor: WidgetStateProperty.resolveWith(
              (states) => SAPColors.primaryBackground),
          empty: SAPAnimationLoaderWidget(
              animation: SAPImages.packageAnimation,
              text: 'Nothing Found',
              height: 200,
              width: 200),
          headingRowDecoration: const BoxDecoration(
            borderRadius: BorderRadius.only(
              topLeft: Radius.circular(SAPSizes.borderRadiusMd),
              topRight: Radius.circular(SAPSizes.borderRadiusMd),
            ),
          ),
          sortArrowBuilder: (bool ascending, bool sorted) {
            if (sorted) {
              return Icon(ascending ? Iconsax.arrow_up_3 : Iconsax.arrow_down,
                  size: SAPSizes.iconSm);
            } else {
              return const Icon(Iconsax.arrow_3, size: SAPSizes.iconSm);
            }
          },
        ),
      ),
